		...
		table => {
			const ArrayCol = [
				table.createDataColumn(row => row, {
					...
					header: t('code'),
					cell: info =>
						CellLinkComponent({ id: info.getValue().id, code: info.getValue().code })
				}),
			...
			if (view === 'OnGoing') {
				ArrayCol.splice(
					...
					table.createDataColumn(
						row => ({ current: row.currentStep, tot: row.steps?.length }),
						{
							id: `step_progress${view}`,
							header: t('step-progress'),
							cell: info => `${info.getValue().current}/${info.getValue().tot}`
						}
					)
				);
			}
