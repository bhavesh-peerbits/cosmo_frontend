...
type SingleControlSelectProps<
	T extends FieldValues,
	TName extends FieldPath<T>
> = UnpackNestedValue<PathValue<T, TName>> extends string
	? {
			label: string;
			name: TName;
...
const SingleControlSelect = <T extends FieldValues, TName extends FieldPath<T>>({
	label,
	control,
	name,
	rules,
	...
	defaultValue,
	controls
}: SingleControlSelectProps<T, TName>) => {
		...
		<>
									...
									{!readOnly &&
										(value ? (
											...
											<Button
												...
												size='sm'
												hasIconOnly
												iconDescription={t('changeMonitoring:select-control')}
								...
								<div className='flex h-full w-full items-center justify-between space-x-2 pl-5 pr-8'>
									{value ? (
										...
										<div className='text-text-placeholder text-body-compact-1'>
											{t('changeMonitoring:select-a-control')}
										</div>
									)}
								</div>
							</Tile>
			...
			<SingleAddSelect
				...
				onSubmit={selectedControl => {
					onChange(controls.find(c => c === selectedControl));
					setOpenSearch(false);
				}}
				onClose={() => setOpenSearch(false)}
				onSubmitButtonText={t('applicationSelect:select')}
				searchResultsLabel={t('applicationSelect:search-results')}
				title={t('changeMonitoring:select-control')}
				description={t('changeMonitoring:select-single-control')}
...
export default SingleControlSelect;
