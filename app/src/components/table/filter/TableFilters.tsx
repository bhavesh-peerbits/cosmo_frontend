...
const TableFilters = <T extends object>({
	...
	return (
		<div>
			...
			<div className='absolute -top-[2px] left-0 flex w-[670px] flex-col justify-end bg-layer-1 shadow-background drop-shadow drop-shadow-2xl'>
				<div className='px-5 pt-5 pb-9'>
					<span className='typography-productive-heading-1 mb-6 block'>Filters</span>
					<div className='grid grid-cols-[1fr,1fr] gap-y-5 gap-x-7'>
						{filterRef &&
							[...filterState.values()].map(f => (
								<FilterElement
									key={f.id}
									column={f.column}
									prefilteredValue={prefilteredValues?.getValue(f.id)}
